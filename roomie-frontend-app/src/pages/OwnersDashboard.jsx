...
import RentPaymentComponent from "../components/RentPaymentComponent";
...
const OwnersDashboard = () => {
  ...
  const updateRepairStatus = async (requestId, newStatus) => {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          `❌ Failed to update status. HTTP Status: ${response.status}`
        );
  ...
  const handleStatusUpdate = async () => {
    ...
    console.log(
      `🔧 Handling status update for request ID: ${selectedRequest.id}`
    );

    try {
      const updatedStatus = await updateRepairStatus(
        selectedRequest.id,
        newStatus
      );
  ...
  const handleSaveField = (propertyId, field) => {
    ...
    fetch(
      `http://127.0.0.1:8000/properties/${propertyId}/update-text-fields/`,
      {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleImageChange = (propertyId, e) => {
    ...
    fetch(`http://127.0.0.1:8000/properties/${propertyId}/`, {
      ...
      .then((updatedProperty) => {
        ...
        setShowImageInput((prev) => ({ ...prev, [propertyId]: false })); 
  ...
  const handleRoomImageChange = (propertyId, index, e) => {
    const updatedProperties = properties.map((property) => {
      if (property.id === propertyId) {
        ...
        const existingImagePublicId =
          existingImage?.image?.split("/").pop().split(".")[0] || null; // Extract the public ID from the existing image URL
    ...
    const existingImagePublicId = updatedProperties.find(
    ...
    if (existingImagePublicId) {
      ...
      console.log(
        "Existing Image Public ID to send to backend: ",
        existingImagePublicId
      );
    }
    ...
    fetch(`http://127.0.0.1:8000/properties/${propertyId}/`, {
      ...
      .catch((error) => {
        setMessage({
          text: "Error updating room image. Please try again.",
          type: "danger",
        });
        setImageUploading(false); // Stop uploading state
      });
  ...
  const handleDeleteRoomImage = async (propertyId, cloudinaryImageUrl) => {
    ...
    try {
      ...
      setMessage({
        text: "Sorry can't remove image right now!",
        type: "error",
      });
      setTimeout(() => {
        setMessage(null);
  ...
  const handleAddImageClick = async (propertyId, imageFile, description) => {
    ...
    if (!description) {
      console.warn(
        "⚠️ No description provided. Defaulting to an empty string."
      );
    }
    ...
    if (!token) {
      console.error(
        "❌ No access token found in localStorage! User may not be authenticated."
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          `❌ Image upload failed. Status: ${response.status}, Response: ${errorText}`
        );
        return;
      }
      ...
      if (!responseData.image_url) {
        console.error(
          "❌ Response does not contain 'image_url'! Backend may not be returning it."
        );
        return;
      }
  ...
  return (
    <div className="owner-dashboard-container mt-5">
      <h2 className="owner-dashboard-title text-center mb-4">
        Owner's Dashboard
      </h2>
      {/* Button to Navigate to Notifications Page */}
      <Button
        variant="primary"
        onClick={() => navigate("/tenancy-request-component")}
      >
        Go to Notifications
      </Button>
      ...
      {message && (
        <Alert variant={message.type} className="message-display">
          {message.text}
        </Alert>
      )}
          ...
          <Row className="owner-dashboard-summary-row">
            {properties.map((property) => (
              <Col
                ...
                lg={4}
                key={property.id}
                className="owner-dashboard-summary-col"
              >
                  ...
                  <Card.Body className="owner-dashboard-card-body">
                    ...
                    <div className="owner-dashboard-card-img-container position-relative">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          handleEditField(property.id, "main_image")
                        }
                      >
                        Edit
                      </Button>
                        ...
                        {editingFields[property.id] === field ? (
                          <div className="owner-dashboard-edit-field">
                            <Form.Control
                              type="text"
                              value={
                                editedValues[property.id]?.[field] ||
                                property[field]
                              }
                              onChange={(e) =>
                                handleFieldChange(
                            ...
                            <Button
                              ...
                              onClick={() =>
                                handleSaveField(property.id, field)
                              }
                            >
                              Save
                            </Button>
                          ...
                          <div className="owner-dashboard-view-field">
                            ...
                            <Button
                              ...
                              onClick={() =>
                                handleEditField(property.id, field)
                              }
                            >
                              Edit
                            </Button>
                    ...
                    <div className="owner-dashboard-image-upload">
                      ...
                      {property.room_images.map((roomImage, index) => (
                        <Col
                          ...
                          md={4}
                          lg={4}
                          className="owner-dashboard-room-image-col"
                        >
                          <div className="owner-dashboard-room-image-container position-relative">
                            ...
                            {activeRoomImageIndex === index &&
                              editingFields[property.id] === "room_images" && (
                                <>
                                  <input
                                    type="file"
                                    onChange={(e) =>
          ...
          <Row>
            {repairRequests.map((request) => (
              <Col
                ...
                md={6}
                lg={4}
                className="repair-request-col mb-3"
              >
                <Card
                ...
                >
                  <Card.Body>
                    <h5>
                      Repair Request for Property: {request.property_address}
                    </h5>
                    <p>
